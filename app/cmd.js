...
const cmd = bot => {
    ...
    bot.onText( q('wifi_home_devices'), async mes => {if (wl(mes))  sendTextPh (bot, mes,  await c.wifi.devices()                );});
    bot.onText( q('wifi_home_reboot'),  async mes => {if (wl(mes))  sendText   (bot, mes,  await c.wifi.reboot()                 );});
    bot.onText( q('wifi_knpl_devices'), async mes => {if (wl(mes))  sendTextPh (bot, mes,  await c.wifi.devices({place: 'knpl'}) );});
    bot.onText( q('wifi_knpl_reboot'),  async mes => {if (wl(mes))  sendText   (bot, mes,  await c.wifi.reboot({place: 'knpl'})  );});
